    ...
    for (j in unique(dataset$version_b)) {
      ...
      if (!(pair1 %in% done)) {
        ...
        if (nrow(obs1) != 0 & nrow(obs2) != 0) {
          ...
          new_row <- data.frame(
            version_a = i, version_b = j, a_in_b = i_in_j,
...
get_wdiff_data <- function(wdiff_otuput) {
  ...
  wdiff_words_final <- as.numeric(gsub(
    " [a-z]{1,} ", "",
    substring(
      wdiff_info, wdiff_words_i,
  ...
  wdiff_common_final_pre <- gsub(
    " [a-z]{1,} ", "",
    substring(
      wdiff_info, wdiff_common_i,
      wdiff_common_i + wdiff_common_n
    )
  )
  wdiff_common_words_final <- strsplit(wdiff_common_final_pre, " ")[[1]][1]
  wdiff_common_percs <- c(
    strsplit(wdiff_common_final_pre, " ")[[1]][2],
  ...
  comp_output <- data.frame(
    words_a = as.numeric(gsub(
      " [a-z]{1,} ", "",
      substring(
        wdiff_info, wdiff_words_i[1],
        wdiff_words_i[1] + wdiff_words_n[1]
      )
    )),
    words_b = as.numeric(gsub(
      " [a-z]{1,} ", "",
      substring(
        wdiff_info, wdiff_words_i[2],
...
get_state_abbr <- function(num_states) {
  abbr <- c(
    "MI", "NE", "MN", "CO", "NJ", "WI", "GA", "FL", "MD", "VA", "TX",
  ...
  numcode <- c(
    23, 35, 33, 62, 12, 25, 44, 43, 52, 40, 49, 13, 48, 21, 14,
