...
make_path <- function(value) {
    return(paste0("cleaned_bill_text/", congress, "/", value, ".txt"))
}
...
bill_id_to_path <- function(bill_id) {
    ...
    return(paste0("data/cleaned_bill_text/", congress, "/", relative, ".txt"))
}

# Load the data
load(paste0("pairwise_comparisons/", congress, "combined_comparisons.RData"))
...
# df to csv
write.csv(df, paste0("pairwise_comparisons/", congress, "/combined_comparisons.csv"), row.names = FALSE)
